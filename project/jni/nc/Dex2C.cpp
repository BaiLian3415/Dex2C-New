...
struct MemberTriple {
    MemberTriple(const char *cls_name, const char *name, const char *sig)
        : class_name_(cls_name ? cls_name : ""),
          member_name_(name ? name : ""),
          signautre_(sig ? sig : "") {}
    ...
    bool operator<(const MemberTriple &member) const {
        int cmp = strcmp(class_name_, member.class_name_);
        if (cmp != 0) return cmp < 0;
        cmp = strcmp(member_name_, member.member_name_);
        if (cmp != 0) return cmp < 0;
        cmp = strcmp(signautre_, member.signautre_);
        return cmp < 0;
    }
};
...
// Increased limit for modern large applications
static const int max_global_reference = 4096;
...
void d2c_filled_new_array(JNIEnv *env, jarray array, const char *type, jint count, ...) {
    ...
    for (int i = 0; i < count; i++) {
        if (ref) {
            env->SetObjectArrayElement((jobjectArray) array, i, (jobject) va_arg(args, jlong));
        } else {
...
int64_t d2c_double_to_long(double val) {
    int64_t result;
    if (val != val) { result = 0; }
    else if (val > static_cast<double>(INT64_MAX)) { result = INT64_MAX; }
    else if (val < static_cast<double>(INT64_MIN)) { result = INT64_MIN; }
    else { result = static_cast<int64_t>(val); }
    return result;
}
...
int64_t d2c_float_to_long(float val) {
    int64_t result;
    if (val != val) { result = 0; }
    else if (val > static_cast<float>(INT64_MAX)) { result = INT64_MAX; }
    else if (val < static_cast<float>(INT64_MIN)) { result = INT64_MIN; }
    else { result = static_cast<int64_t>(val); }
    return result;
}
...
int32_t d2c_double_to_int(double val) {
    int32_t result;
    if (val != val) { result = 0; }
    else if (val > static_cast<double>(INT32_MAX)) { result = INT32_MAX; }
    else if (val < static_cast<double>(INT32_MIN)) { result = INT32_MIN; }
...
int32_t d2c_float_to_int(float val) {
    int32_t result;
    if (val != val) { result = 0; }
    else if (val > static_cast<float>(INT32_MAX)) { result = INT32_MAX; }
    else if (val < static_cast<float>(INT32_MIN)) { result = INT32_MIN; }
    else { result = static_cast<int32_t>(val); }
    return result;
}
...
bool d2c_check_cast(JNIEnv *env, jobject instance, jclass clz, const char *class_name) {
    if (instance == NULL) {
        // null can be cast to any reference type in Java
        return false;
    }
...
Java_amimo_dcc_DccApplication__0003cinit_0003e__(JNIEnv *env, jobject thiz) {
    ...
    if (application == NULL) {
        LOGD("Failed to find android/app/Application");
        return;
    }
    jmethodID init = env->GetMethodID(application, "<init>", "()V");
    if (init != NULL) {
        env->CallVoidMethodA(instance, init, NULL);
    }
...
JNIEXPORT jint JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    if (!clz) {
        LOGD("DccApplication class not found");
        return JNI_ERR;
    }
    env->DeleteLocalRef(clz);
    ...
    if (result != nullptr) {
        LOGD("dynamic_register_compile_methods failed: %s", result);
        return JNI_ERR;
    }
