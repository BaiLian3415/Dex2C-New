...
from dex2c.util import get_type_descriptor, get_native_type, JniLongName, is_primitive_type,     get_cdecl_type, get_type
...
class Writer(object):
    ...
    def write_method(self):
        ...
        if self.irmethod.params_type:
            proto = ', '.join(['%s p%s' % (get_native_type(p_type), param) for p_type, param in
        ...
        if return_type[0] != 'V':
            # FIX: handle array return types correctly
            if return_type[0] == 'L' or return_type[0] == '[':
        ...
        if is_primitive_type(elem_type):
            ...
            self.write('env->Set%sArrayRegion((%sArray) v%s, (jint) %s, 1, &val);' %
                       (get_type_descriptor(elem_type), get_native_type(elem_type),
                        self.ra(array), self.get_variable_or_const(index)))
            self.write('}\n')
        else:
    ...
    def _invoke_common(self, ins, invoke_type, name, base, ptype, rtype, args, clsdesc):
        ...
        if invoke_type == 'super':
            self.write('env->CallNonvirtual%sMethodA(v%s, clz, mid, args);\n' % (get_type_descriptor(rtype), self.ra(base)))
    ...
    def visit_check_cast(self, ins, arg, atype):
        ...
        self.write('jclass &clz = %s;\n' % (self.ca(ins.get_class())))
        self.write('D2C_RESOLVE_CLASS(clz,"%s");\n' % (get_type(atype)))
        # FIX: use correct variable reference for check-cast
    ...
    def visit_new_array(self, ins, result, atype, size):
        ...
        if is_primitive_type(elem_type):
            result.visit(self)
            self.write(' = (%s) env->New%sArray((jint) %s);\n' % (
    ...
    def visit_fill_array(self, ins, array, value):
        ...
        self.write('env->Set%sArrayRegion((%sArray) v%s, 0, %d, (const %s *) data);\n' % (
            get_type_descriptor(elem_type), get_native_type(elem_type), self.ra(array), array_size,
    ...
    def visit_get_instance(self, ins, result, arg, ftype, clsdesc, name):
        ...
        self.write('D2C_RESOLVE_FIELD(clz, fld, "%s", "%s", "%s");\n' % (get_type(clsdesc), name, ftype))
        result.visit(self)
        self.write(' = (%s) env->Get%sField(v%s,fld);\n' % (
